<template>
  <div class="container px-8 mx-auto">
    <div class="flex justify-between py-8">
      ...
      <div class="flex items-center space-x-2">
        <DropdownSort @sort="sortTodo" />
        <Button text="+ Tambah" @click="openModal" />
        <Modal ref="modal" v-slot="{ close }">
          <div class="space-y-2">
            <div class="border-b border-gray-200 flex justify-between p-2">
              <h3 class="text-xl font-bold">Tambah List Item</h3>
              <span @click="close" class="text-xl cursor-pointer">&times;</span>
            </div>
            <div class="py-6 space-y-6">
              <div class="space-y-4">
                <label for="">Nama List Item</label>
                <input type="text" class="form-control h-10" v-model="form.title">
              </div>
              <div class="space-y-4">
                <label for="">Priority</label>
                <div class="w-48">
                  <select name="" class="form-control h-10" id="" v-model="form.priority">
                    <option :value="i.id" v-for="i in priorities" :key="i.id">
                      <div class="flex">
            ...
            <div class="mx-auto w-full space-x-2 text-right border-t p-2 py-4">
              <button class="p-2 px-8 bg-blue-500 rounded-4xl text-white text-lg" @click="() => {addOne(); close();}">
                Simpan
              </button>
            </div>
          </div>
